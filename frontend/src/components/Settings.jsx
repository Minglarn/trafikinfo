...
export default function Settings() {
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-8 pb-20 relative">
            {!isLoggedIn && (
                <div className="absolute inset-0 z-10 bg-slate-50/50 dark:bg-slate-950/50 backdrop-blur-[2px] rounded-3xl flex items-center justify-center p-8 text-center">
                    <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 max-w-sm">
                        <div className="w-16 h-16 bg-blue-500/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <ShieldCheck className="w-8 h-8 text-blue-500" />
                        </div>
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Inställningar låsta</h3>
                        <p className="text-slate-500 dark:text-slate-400 mb-6">
                            Du måste logga in som Admin för att kunna ändra inställningar eller återställa systemet.
                        </p>
                    </div>
                </div>
            )}

            <div className={!isLoggedIn ? 'opacity-40 grayscale-[0.5] pointer-events-none select-none' : ''}>
                <div>
